  ...
  try {
    const response = await client.messages.create({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: `Today is ${new Date().toISOString().slice(0, 10)}. Traveler profile: ${profileCtx}. Based on this travel planning conversation, return ONLY a raw JSON object — no markdown, no code blocks. Use exactly this structure (set overview or strategy to null if insufficient travel details yet):
...
async function downloadTripPDF(intelligence, realResults, flightPointsAdvice, hotelPointsAdvice) {
  const JsPDF = await loadJsPDF();
  const { overview, strategy, checklist } = intelligence ?? {};
  const doc = new JsPDF({ unit: 'pt', format: 'letter' });

  // ── Palette ──────────────────────────────────────────────────────────────
  const gold    = [201, 168, 76];
  const dark    = [15,  21,  38];
  const card    = [22,  33,  58];   // slightly lighter card bg
  const white   = [255, 255, 255];
  const dim     = [130, 148, 175];
  const goldDim = [160, 130, 60];

  const W      = doc.internal.pageSize.getWidth();   // 612
  const pageH  = doc.internal.pageSize.getHeight();  // 792
  const ML     = 40;   // margin left
  const MR     = 40;   // margin right
  const CW     = W - ML - MR;  // content width 532
  const FOOTER = 32;  // footer height reserve
  let y = 0;
  let pageNum = 1;
  ...
  const drawHeader = (isFirst) => {
    if (isFirst) {
      ...
      doc.setFontSize(8.5);
      doc.text('AI Travel Concierge', ML + 56, 33);
    } else {
      // Thin gold bar for continuation pages
      doc.setFillColor(...gold);
      doc.rect(0, 0, W, 28, 'F');
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(9);
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7.5);
      doc.text(`Page ${pageNum}`, W - MR - 30, 18);
    }
  };
  ...
  const drawFooter = () => {
    ...
    doc.line(ML, pageH - FOOTER + 2, W - MR, pageH - FOOTER + 2);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(...dim);
    doc.text('Generated by Flio  ·  flio-six.vercel.app', ML, pageH - FOOTER + 14);
    doc.text(`${pageNum}`, W - MR, pageH - FOOTER + 14, { align: 'right' });
  };
  ...
  const checkBreak = (needed) => {
    if (y + needed > pageH - FOOTER - 8) addPage();
  };
  ...
  const sectionLabel = (text) => {
    ...
    // Letter-spacing effect by spacing chars manually isn't trivial in jsPDF, so just uppercase
  ...
  const bullet = (text, color = white, indent = ML + 12) => {
    const textW = CW - (indent - ML) - 4;
    const lines = doc.splitTextToSize(text, textW);
    const rowH = lines.length * 12 + 3;
    checkBreak(rowH);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
  ...
  const numberedItem = (n, text, color = white) => {
    const indent = ML + 18;
    const textW  = CW - 18 - 4;
    const lines  = doc.splitTextToSize(text, textW);
    const rowH   = lines.length * 12 + 3;
  ...
  if (overview?.destination) {
    ...
    doc.text(overview.destination, ML, y);
    y += 20;

    const meta = [overview.dates, overview.cabinClass].filter(Boolean).join('   ·   ');
    if (meta) {
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.setTextColor(...dim);
  ...
  if (overview?.recommendation) {
    gap(4);
    const lines = doc.splitTextToSize(overview.recommendation, CW);
  ...
  // ── Best Card ─────────────────────────────────────────────────────────────
  if (overview?.bestCard?.name) {
    sectionLabel('Recommended Card');
    row('Card', overview.bestCard.name, dim, gold);
    if (overview.bestCard.reason) row('Why', overview.bestCard.reason, dim, white);
    gap(12);
  }

  // ── Points Strategy ───────────────────────────────────────────────────────
  if (overview?.pointsUsed?.length) {
    sectionLabel('Points Strategy');
    overview.pointsUsed.forEach((p) => {
      row(p.program, `${p.amount?.toLocaleString() ?? '—'} pts  →  $${p.dollarValue?.toLocaleString() ?? '—'} value`);
    });
    if (strategy?.pointsToUse?.reasoning) {
      gap(4);
      bullet(strategy.pointsToUse.reasoning, dim);
    }
    if (strategy?.pointsToSave?.amount != null) {
      row('Points to keep', `${strategy.pointsToSave.amount.toLocaleString()} pts — ${strategy.pointsToSave.reasoning ?? ''}`, dim, dim);
    }
    gap(12);
  }
  ...
  // ── Booking Sequence ──────────────────────────────────────────────────────
  if (strategy?.transferPartners?.length) {
    sectionLabel('Booking Steps');
    strategy.transferPartners.forEach((step, i) => numberedItem(i + 1, step));
    gap(12);
  }

  // ── Perks ─────────────────────────────────────────────────────────────────
  if (strategy?.perks?.length) {
    sectionLabel('Perks That Apply');
    strategy.perks.forEach((p) => bullet(p, white));
    gap(12);
  }
  ...
  if (strategy?.warnings?.length) {
    ...
    strategy.warnings.forEach((w) => bullet(w, [220, 100, 80]));
    gap(12);
  }

  // ══════════════════════════════════════════════════════════════════════════
  // CHECKLIST — one section group per page region
  // ══════════════════════════════════════════════════════════════════════════
  if (checklist?.sections?.length) {
    checkBreak(60);
    sectionLabel('Pre-Departure Checklist');
    ...
    checklist.sections.forEach((section) => {
      ...
      section.items?.forEach((item) => {
        const textColor = item.checked ? dim : white;
  ...
  // FLIGHTS
  // ══════════════════════════════════════════════════════════════════════════
  const flights = realResults?.flights ?? [];
  if (flights.length > 0) {
    checkBreak(60);
    ...
    flights.forEach((flight, i) => {
      ...
      const airline  = first.airline || 'Unknown Airline';
      const dep      = first.departure_airport || {};
      const arr      = last.arrival_airport   || {};
      const stops    = Math.max(0, legs.length - 1);
      const stopsTxt = stops === 0 ? 'Nonstop' : `${stops} stop${stops !== 1 ? 's' : ''}`;
      const mins     = flight.total_duration;
      const duration = mins ? `${Math.floor(mins / 60)}h ${mins % 60}m` : '';
      const price    = flight.price != null ? `$${flight.price.toLocaleString()}` : '';

      // Card block
      const cardH = 68 + (flightPointsAdvice?.[i] ? 22 : 0);
      checkBreak(cardH + 8);
      doc.setFillColor(...card);
      doc.roundedRect(ML, y - 4, CW, cardH, 4, 4, 'F');

      doc.setFont('helvetica', 'bold');
      doc.setFontSize(9.5);
      doc.setTextColor(...white);
      doc.text(`${airline}`, ML + 10, y + 10);

      if (price) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(11);
        doc.setTextColor(...gold);
        doc.text(price, ML + CW - 10, y + 10, { align: 'right' });
      }

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.setTextColor(...dim);
      const route = [dep.id && arr.id ? `${dep.id}  →  ${arr.id}` : '', stopsTxt, duration].filter(Boolean).join('   ·   ');
      doc.text(route, ML + 10, y + 24);

      if (dep.time || arr.time) {
        const times = [dep.time, arr.time].filter(Boolean).join('  →  ');
        doc.setFontSize(8.5);
        doc.text(times, ML + 10, y + 37);
      }

      if (flightPointsAdvice?.[i]) {
        doc.setFont('helvetica', 'italic');
        doc.setFontSize(8);
        doc.setTextColor(...gold);
        const aLines = doc.splitTextToSize(`Points tip: ${flightPointsAdvice[i]}`, CW - 20);
        doc.text(aLines, ML + 10, y + 52);
        y += cardH + 10;
      } else {
        y += cardH + 10;
      }
  ...
  // HOTELS
  // ══════════════════════════════════════════════════════════════════════════
  const hotels = realResults?.hotels ?? [];
  if (hotels.length > 0) {
    checkBreak(60);
    sectionLabel('Hotels');
    ...
    hotels.forEach((hotel, i) => {
      ...
      const rating  = hotel.overall_rating ? `${hotel.overall_rating} / 5` : '';
      const reviews = hotel.reviews ? `${hotel.reviews.toLocaleString()} reviews` : '';
      const stars   = hotel.hotel_class ? `${'*'.repeat(Math.min(5, hotel.hotel_class))} (${hotel.hotel_class}-star)` : '';

      const cardH = 68 + (hotelPointsAdvice?.[i] ? 22 : 0);
      checkBreak(cardH + 8);
      doc.setFillColor(...card);
      doc.roundedRect(ML, y - 4, CW, cardH, 4, 4, 'F');
      ...
      doc.setFontSize(9.5);
      doc.setTextColor(...white);
      const nameLines = doc.splitTextToSize(hotel.name || 'Hotel', CW - 100);
      doc.text(nameLines, ML + 10, y + 10);
      ...
      if (price) {
        ...
        doc.setFontSize(7.5);
        doc.setTextColor(...dim);
        doc.text('/night', ML + CW - 10, y + 20, { align: 'right' });
      }

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8.5);
      doc.setTextColor(...dim);
      const meta = [stars, rating, reviews].filter(Boolean).join('   ·   ');
      if (meta) doc.text(meta, ML + 10, y + 26);

      if (hotelPointsAdvice?.[i]) {
        doc.setFont('helvetica', 'italic');
        doc.setFontSize(8);
        doc.setTextColor(...gold);
        const aLines = doc.splitTextToSize(`Points tip: ${hotelPointsAdvice[i]}`, CW - 20);
        doc.text(aLines, ML + 10, y + 43);
        y += cardH + 10;
      } else {
        y += cardH + 10;
      }
  ...
  doc.save(`flio-${dest}${date ? '-' + date : ''}.pdf`);
}
...
function TripIntelligencePanel({ intelligence, loading, activeTab, onTabChange, newIndicators, onClearIndicator, onToggleChecklistItem, realResults, realResultsLoading, realResultsError, flightPointsAdvice, hotelPointsAdvice, isLight = false }) {
  ...
  const handleDownloadPDF = async () => {
    setPdfLoading(true);
    try {
      await downloadTripPDF(intelligence, realResults, flightPointsAdvice, hotelPointsAdvice);
    } finally {
      setPdfLoading(false);
    }
  };
